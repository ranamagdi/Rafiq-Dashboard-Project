...
const StatusBadge = ({ status }: { status: string }) => {
  const map: Record<string, { label: string; style: string; dotColor: string }> = {
    TODO: {
      label: "Todo",
...
const DetailsTask = ({ isOpen, onClose, projectId, taskId }: DetailsTaskProps) => {
...
useEffect(() => {
  if (isOpen && !prevOpenRef.current) {
    setFetchKey((k) => k + 1); // only setState inside effect via updater
  }
...
}, [fetchKey, isOpen, projectId, setFetchState, taskId]);
  return (
    <>
      ...
      >
        ...
        {fetchState.status === "error" && (
          <div className="p-10 text-center text-red-500">{fetchState.message}</div>
        )}
                ...
                <div className="grid grid-cols-2 gap-3">
                  ...
                  {task.due_date && (
                    <div className="bg-[#F1F3FF] rounded-2xl p-3 flex flex-col gap-2">
                      ...
                      <div className="flex items-center gap-1.5">
                        <span className="text-gray-400 text-sm"><CalendarIcon /></span>
                  ...
                  {task.created_at && (
                    <div className="bg-[#F1F3FF] rounded-2xl p-3 flex flex-col gap-2">
                      ...
                      <div className="flex items-center gap-1.5">
                        <span className="text-gray-400 text-sm"><Clock /></span>
                        <p className="text-sm font-medium text-gray-800">
                ...
                <div className="flex-1 p-8 overflow-y-auto border-r border-gray-200">
                  <div className="flex items-center gap-2 mb-4 text-xs text-gray-500">
                    ...
                    {task.epic_id && (
                      <>
                        <span><EpicId /></span>
                        <span className="text-gray-600">Epic-{task.epic_id}</span>
                      </>
                    )}
                  </div>
                  ...
                  <hr className="border-[#C3C6D6]/20 mt-1" />
