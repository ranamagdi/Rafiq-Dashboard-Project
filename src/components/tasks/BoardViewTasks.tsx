...
function BoardColumn({
  ...
  const { items: colTasks, loading, hasMore, lastElementRef } = usePagination<Task>({
    mode: "infinite",
      ...
      <div className="flex items-center justify-between mb-3">
        <div className="flex items-center gap-2">
          ...
          <span className={`text-[11px] font-semibold px-2 py-0.5 rounded-sm ${col.badgeClass}`}>
            {loading && colTasks.length === 0 ? "..." : colTasks.length}
          </span>
        </div>
        <Button
          className="bg-transparent text-gray-400 shadow-transparent px-0"
          onClick={() => navigate(`/dashboard/project/${projectId}/tasks/new?status=${col.status}`)}
        >
          <PlusIcon />
        </Button>
      </div>
      ...
      {loading && colTasks.length === 0 ? (
        ...
        <>
          {colTasks.map((task, index) => {
            const isLast = index === colTasks.length - 1;
            return (
              <div
                key={task.id}
                className="mb-2.5"
              ...
              >
                <BoardViewCard
                  ...
                  taskId={task.id}
                  projectId={projectId}
                  onClick={(taskId, projectId) => setSelectedTask({ taskId, projectId })}
                />
              </div>
            );
          })}
...
export default function BoardViewTasks({ projectId }: BoardViewTasksProps) {
  return (
    
      <div className="flex-1 overflow-x-auto">
        <div className="grid grid-cols-8 gap-6 min-w-max">
          {COLUMNS.map((col) => (
            <BoardColumn key={col.id} col={col} projectId={projectId} />
          ))}
        </div>
      </div>
